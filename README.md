...
## How it works
- **Colour** — a seated desk takes its occupant's team as its fill, border and label tone; the rail chip carries the same wash, so the list and the floor read as one thing. Empty desks are dashed and quiet, and someone seated with no team recorded gets a neutral fill. Teams hold a *hue*, not a fixed colour: the theme decides how each hue resolves, so light gets pale washes with a saturated edge and dark gets a deeper fill with a softer edge.
- **Teams** — the Team field is a typeahead over the teams already in the plan, in both the add-person row and the rename dialog. It matches anywhere in the name (`form` finds `Platform`), ranks exact prefixes first and then by headcount, shows how many people are on each, and offers *Add team X* when what you typed is new. Arrow keys move, Enter picks, Escape closes; Enter with nothing highlighted still submits the form. Typing `platform` when `Platform` exists reuses the existing team rather than creating a near-duplicate with its own colour.
